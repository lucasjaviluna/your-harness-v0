...
- Si existe una tarea `simple` activa y llega otro prompt normal, la TUI debe permitir ampliar la tarea actual o crear una tarea independiente. Al elegir ampliar, el nuevo texto se incorpora como alcance y se ejecuta la reevaluación de complejidad.
...
- Una modificación del alcance vuelve a evaluar la complejidad de la tarea. Las rutas pueden escalar `simple → task → sdd`; la nueva ruta y el plan requieren revisión HIL antes de continuar. Una tarea ya escalada no se degrada por una edición posterior. Un `--mode` manual controla el inicio, pero no bloquea escaladas de seguridad.
...
- Cuando una reevaluación cambia la complejidad, la cabecera debe mostrar la evidencia inmediata del cambio: `Esta tarea fue reevaluada: <ruta anterior> -> <ruta nueva>`.
